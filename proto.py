...
def read_binary_file(file_name):
    # define the dictionary for enum record types
    enum_record_types = {
        0x00: 'Debit',
        0x01: 'Credit',
        0x02: 'StartAutopay',
    ...
    with open(file_name, 'rb') as f:
        ...
        if magic_string == 'MPS7':  # Validating the magic string to ensure we are parsing the correct file format
            for _ in range(num_records):
                ...
                record_type_str = enum_record_types.get(record_type, 'Unknown')
                ...
                if record_type == 0x00:  # Debit
                    ...
                    if user_id == user_id_to_find_balance:
                        user_id_credit += cost  # obtaining the total debit for user id 2456938384156277127
                elif record_type == 0x02:
                    dict_output_totals['autopays started'] += 1
                elif record_type == 0x03:
                    dict_output_totals['autopays ended'] += 1
    for k, v in dict_output_totals.items():
        print(f'{k}={v}')
    print(f'balance for user id {user_id_to_find_balance}={user_id_credit - user_id_debit}')


# Call the function passing the file
read_binary_file('txnlog.dat')
